...
// Assets are located in the public folder
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }) {
  ...
  const dragVec = useRef(new THREE.Vector3());
  ...
  useFrame((state, delta) => {
    if (dragged && card.current) {
      ...
      card.current.setNextKinematicTranslation({ x: vec.current.x - dragged.x, y: vec.current.y - dragged.y, z: vec.current.z - dragged.z });
    }
    if (fixed.current && j1.current && j2.current && j3.current && card.current && band.current) {
      [j1, j2].forEach((ref: any) => {
        const trans = ref.current.translation();
        if (isNaN(trans.x) || isNaN(trans.y) || isNaN(trans.z)) return;
      ...
      if (isNaN(j3Trans.x) || isNaN(fixedTrans.x) || isNaN((j2.current as any).lerped?.x) || isNaN((j1.current as any).lerped?.x)) return;
      ...
      cardQuat.current.set(cardRotation.x, cardRotation.y, cardRotation.z, cardRotation.w);
      cardEuler.current.setFromQuaternion(cardQuat.current);
      
      const cardAngVel = card.current.angvel();
      ang.current.set(cardAngVel.x, cardAngVel.y, cardAngVel.z);
      rot.current.set(cardEuler.current.x, cardEuler.current.y, cardEuler.current.z);
          ...
          <group
            ...
            onPointerUp={(e: any) => (e.target.releasePointerCapture(e.pointerId), drag(false))}
              onPointerDown={(e: any) => (
                ...
                drag(dragVec.current.copy(e.point).sub(vec.current.copy(card.current?.translation() as THREE.Vector3)))
              )}
          >
